// src/api.js - Enhanced version with top artists and Last.fm integration
...
export class LastFmApi {
  ...
  /**
   * Match Spotify artist to Last.fm using name and potentially MBID
   * 
   * @param {SpotifyArtist} spotifyArtist
   */
  async matchSpotifyArtist(spotifyArtist) {
    // First try exact name match
    const tags = await this.getArtistTags(spotifyArtist.name)
    const similarArtists = await this.getSimilarArtists(spotifyArtist.name, 20)
    
    return {
      name: spotifyArtist.name,
      tags: tags.slice(0, 5), // Top 5 tags
      similarArtists: similarArtists.slice(0, 10), // Top 10 similar
